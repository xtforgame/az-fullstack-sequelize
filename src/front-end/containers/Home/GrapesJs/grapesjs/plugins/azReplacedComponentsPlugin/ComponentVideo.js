...
export default (editor, options) => {
  const { model: Component } = editor.DomComponents.getType('image');
  return Component.extend(
    {
      ...
      initialize(o, opt) {
        this.em = opt.em;
        if (o?.attributes?.src) {
          this.parseFromSrc(o?.attributes?.src);
        } else if (this.get('src') && this.get('src') !== this.get('fallback')) {
          this.parseFromSrc();
        }
        this.updateTraits();
       ...
       */
      parseFromSrc(src) {
        const src2 = src || this.get('src');
        const prov = this.get('provider');
        const uri = this.parseUri(src2);
        const qr = uri.query;
        switch (prov) {
          case yt:
          case ytnc:
